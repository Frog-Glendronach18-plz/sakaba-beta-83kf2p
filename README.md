<!-- 画像はリポジトリ内の img/ を相対パスで参照している。
     README を差し替えるときは img/ogp.jpg も一緒にアップロードすること（無いと画像が割れる）。
     ogp.jpg は index.html の og:image（SNS共有プレビュー）と共用。1200×630。
     原寸のキービジュアルは design/ogp_src.png（1730×909。design/ は配布に含めない）。 -->

<div align="center">

<img src="img/ogp.jpg" alt="酒場ドラフト戦記" width="820">
...
![登録不要](https://img.shields.io/badge/%E4%BC%9A%E5%93%A1%E7%99%BB%E9%8C%B2-%E4%B8%8D%E8%A6%81-37a05a?style=flat-square)
...
### 🍀 28種類の「旅立ちイベント」
ゲーム開始時、3つの選択肢から1つを選びます。これがその冒険の方向性を決めます。
...
🌾 **草原** 特別な効果のない戦場 ／ ⛰️ **山岳** 岩山が進路を阻む
❄️ **雪原** 足を取られ移動が鈍る ／ 🔥 **魔界** 魔王の待つ最終決戦地

### 🏟️ 闘技場 — 他プレイヤーの軍団と戦う（非同期対戦）
完走した部隊は **軍団として登録** でき、他のプレイヤーの軍団と戦えます。
相手はあなたが挑んだ時点の記録から再現されるので、**相手のオンライン状況は関係ありません**。

- 🌐 **世界中の挑戦者** — 軍団を公開すると、他のプレイヤーの軍団が抽選で相手に。勝敗は**ランキング**にも反映されます
- 🔗 **挑戦コード / 挑戦リンク** — 自分の軍団をSNSやLINEで友達に送りつけて戦わせることも

> オンライン機能は**すべて任意**です。使わなくても全モードが遊べます（アカウント登録も不要）。

### 🎨 手描きのドット絵
仲間も魔物も、1体ずつ描き起こしたドット絵。盤面では円のアイコンではなく**立ち絵がそのまま地形に立ちます**。
絵文字表示（クラシック）に戻すこともできます。

### 📱 その他
- **PWA対応** — ホーム画面に追加すればオフラインでも遊べます
- **中断・再開** — 途中でブラウザを閉じても、次に開けば続きから
- **縦持ち / 横持ち** — 横向きにすると2カラムになり、盤面と酒場を同時に見られます（設定で固定も可）
...
- **引き付け**: 衛兵・ゴーレム・城塞兵は優先的に狙われます。前に置いて壁にしましょう
